...
function renderCategories() {
  ...
  const labels = { all: 'All Products', interlock: 'Interlock', natural: 'Natural Stone', blocks: 'Blocks' };
  const container = document.getElementById('categoryBtns');
  container.innerHTML = cats.map(c => `
    <button class="cat-btn ${c === 'all' ? 'active' : ''}" data-cat="${c}">
      ${labels[c] || c}
    </button>
  `).join('');
      ...
      <div class="card-body">
        ...
        <div class="card-footer">
          <div class="price-stack">
            ${p.priceWithPolish > 0 ? `<span class="price-tag">₹${p.priceWithPolish}/sqft with polish</span>` : ''}
            <span class="price-polish">${p.priceWithoutPolish > 0 ? '₹' + p.priceWithoutPolish + '/sqft without polish' : 'Contact for price'}</span>
          </div>
