...
async function loadColors() {
  ...
  try {
    ...
    if (snap.empty) {
      list.innerHTML = '<p class="empty-msg">No colors yet. Add one above.</p>';
      return;
    }
    list.innerHTML = snap.docs.map(doc => itemHTML(doc.id, doc.data().name)).join('');
  } catch (err) {
    list.innerHTML = '<p class="empty-msg">Error loading colors.</p>';
...
async function addColor() {
  const input = document.getElementById('newColorName');
  const name = input.value.trim();
  if (!name) { showToast('Enter a color name.', 'error'); return; }
  try {
    const ref = await db.collection(COL).add({ name });
    document.getElementById('colorList').insertAdjacentHTML('afterbegin', itemHTML(ref.id, name));
    input.value = '';
...
async function deleteItem(id, name) {
  ...
  try {
    ...
    const list = document.getElementById('colorList');
    if (!list.children.length) list.innerHTML = '<p class="empty-msg">No colors yet. Add one above.</p>';
